...
export function updateUserActive(userId) {
  if (!userId) return
...
export function createSignup({ name, email, number, numberHash, passwordHash, verificationToken, tokenExpires }) {
...
export function getSignupByToken(token) {
  return db.prepare(`
    SELECT * FROM signups WHERE verification_token = ?
  `).get(token)
}

export function getSignupByEmail(email) {
  return db.prepare(`
...
export function updateVerificationToken(id, token, tokenExpires, passwordHash) {
  if (passwordHash) {
    db.prepare(`
      UPDATE signups SET verification_token = ?, token_expires = ?, password_hash = ? WHERE id = ?
    `).run(token, tokenExpires, passwordHash, id)
  } else {
    db.prepare(`
      UPDATE signups SET verification_token = ?, token_expires = ? WHERE id = ?
    `).run(token, tokenExpires, id)
  }
}

export function markSignupVerified(id) {
  db.prepare(`
    UPDATE signups SET verified = 1, verification_token = NULL WHERE id = ?
  `).run(id)
}

export function deleteSignup(id) {
  db.prepare('DELETE FROM messages WHERE user_id = ?').run(id)
  return db.prepare(`DELETE FROM signups WHERE id = ?`).run(id)
}

export function addMessage(userId, message, type = 'text', attachmentPath = null, senderType = 'user') {
  const result = db.prepare(`
...
export function markSupportMessagesAsSeen(userId) {
  db.prepare(`
    UPDATE messages SET status = 'seen' WHERE user_id = ? AND sender_type = 'support' AND (status IS NULL OR status != 'seen')
  `).run(userId)
}

export function markUserMessagesAsSeen(userId) {
  db.prepare(`
    UPDATE messages SET status = 'seen' WHERE user_id = ? AND sender_type = 'user' AND (status IS NULL OR status != 'seen')
  `).run(userId)
}
...
export function createFeedPost(title, content, imageUrl = null, videoUrl = null) {
  const result = db.prepare(`
...
export function updateFeedPost(id, title, content, imageUrl = undefined, videoUrl = undefined) {
  ...
  return db.prepare(`
    UPDATE feed_posts SET title = ?, content = ?, image_url = ?, video_url = ?, date = ? WHERE id = ?
  `).run(title, content, img, vid, new Date().toISOString(), id)
}

export function deleteFeedPost(id) {
  return db.prepare('DELETE FROM feed_posts WHERE id = ?').run(id)
}
...
export function getAdminStats() {
  ...
  const recentSignups = db.prepare(`
    SELECT id, name, email, verified, date FROM signups ORDER BY date DESC LIMIT 5
  `).all()
  return {
...
export function addAdminFriend(adminId, friendAdminId) {
...
export function areAdminFriends(adminId, friendAdminId) {
...
export function sendFriendRequest(requesterId, recipientId) {
  if (requesterId === recipientId) return null
  if (areFriends(requesterId, recipientId)) return null
  if (hasPendingFriendRequest(requesterId, recipientId)) return null
  try {
    const result = db.prepare(`
      INSERT INTO friend_requests (requester_id, recipient_id, status, date) VALUES (?, ?, 'pending', ?)
    `).run(requesterId, recipientId, new Date().toISOString())
    return result.lastInsertRowid
  } catch {
    return null
  }
...
export function hasPendingFriendRequest(requesterId, recipientId) {
...
export function getPendingRequestsReceived(userId) {
  return db.prepare(`
...
export function rejectFriendRequest(requestId, recipientId) {
  const req = getFriendRequestById(requestId)
...
export function getPendingRequestIdsSent(userId) {
  const rows = db.prepare(`
    SELECT recipient_id FROM friend_requests WHERE requester_id = ? AND status = 'pending'
  `).all(userId)
  return new Set(rows.map(r => r.recipient_id))
}

export function getPendingRequestIdsReceived(userId) {
  const rows = db.prepare(`
    SELECT requester_id FROM friend_requests WHERE recipient_id = ? AND status = 'pending'
  `).all(userId)
...
export function removeFriend(userId, friendId) {
  ...
  const admin = getAdminByEmail(friend.email)
...
export function addDirectMessage(senderId, recipientId, message, type = 'text', attachmentPath = null) {
  return db.prepare(`
...
export function markDirectMessagesAsSeen(viewerId, otherUserId) {
  db.prepare(`
    UPDATE direct_messages SET status = 'seen'
    WHERE sender_id = ? AND recipient_id = ? AND (status IS NULL OR status != 'seen')
  `).run(otherUserId, viewerId)
}

export function getDirectMessages(userId, friendId) {
...
export function createGame(name, link = null, logoPath = null) {
  const maxOrder = db.prepare('SELECT COALESCE(MAX(sort_order), -1) + 1 as next FROM games').get()
  const result = db.prepare(`
    INSERT INTO games (name, link, logo_path, sort_order) VALUES (?, ?, ?, ?)
  `).run(name.trim(), link?.trim() || null, logoPath, maxOrder.next)
  return result.lastInsertRowid
}

export function updateGame(id, name, link = null, logoPath = null, clearLogo = false) {
  const existing = getGameById(id)
  if (!existing) return false
  const finalLogo = clearLogo ? null : (logoPath ?? existing.logo_path)
  db.prepare(`
    UPDATE games SET name = ?, link = ?, logo_path = ? WHERE id = ?
  `).run(name.trim(), link?.trim() || null, finalLogo, id)
  return true
}
    ...
    const transaction = db.transaction((items) => {
      for (const s of items) {
        insert.run(
          ...
          s.verified ? 1 : 0,
          s.verificationToken || s.verification_token || null,
          null
        )
      }
    })
